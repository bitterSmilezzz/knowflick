...
在应用内点击右上角「偏好设置」（齿轮图标），在「AI 驱动服务」卡片中选择服务商：

| 服务商预设 | 默认 Base URL | 推荐模型 | 说明 |
| --- | --- | --- | --- |
| **DeepSeek** | `https://api.deepseek.com` | `deepseek-chat`, `deepseek-reasoner` | 官方高性价比模型，填入 API Key 即可 |
...
| **智谱 GLM** | `https://open.bigmodel.cn/api/paas/v4` | `glm-4-flash`, `glm-4-plus` | 智谱 AI 开放平台，填入 Key 即可 |
| **OpenAI** | `https://api.openai.com/v1` | `gpt-4o-mini`, `gpt-4o` | 官方 GPT 系列端点，填入 Key 即可 |
| **Ollama (本地)** | `http://localhost:11434/v1` | `llama3.2`, `qwen2.5`, `deepseek-r1` | 本地私有离线运行，**无需 API Key** |
| **自定义** | 自定义端点地址 | 自定义模型名 | 适配任意第三方 OpenAI 兼容端点或中转站 |
