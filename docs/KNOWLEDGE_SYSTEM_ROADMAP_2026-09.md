...
- **mac 后台播控 🟡 代码完成、待 CI 编译与下个版本发布**：`MPNowPlayingInfoCenter` + `MPRemoteCommandCenter`（播放/暂停/上下曲/±10 秒）已落在 Core 并由 7 项新用例覆盖；App 层只需一行 `enableNowPlaying()`。
  两个设计决定值得记下来：
  ① **系统接入默认关闭**，由 App 显式打开。控制中心与媒体键是**进程级**资源，Core 单测跑在非 App 进程里，
     顺手注册就会抢走开发者当前会话的媒体键路由——这类副作用编译器完全看不见。
  ② 进度不按时钟回写：快照里带 `playbackRate`，系统自行推进进度条，所以只在状态跳变（播/停/切卡/seek）时发布一次。

**一条被推翻的"实测发现"（记下来防重犯）**：我一度认定"关掉最后一个窗口后进程不退出、但 Dock reopen 不重建窗口，应用变成有声音没界面"，
并为此写了 `AppDelegate.applicationShouldHandleReopen`。随后拿 CI 产物做行为验证时发现：**刚启动、窗口明明在屏上，System Events 的窗口计数也返回 0**——
因为这台机器的会话当时是锁屏状态，测量仪器从一开始就是坏的，那个"陷阱"根本没被观察到。
已把 AppDelegate 撤掉（不在未证实的前提上加代码），reopen 是否真的有问题留到一次解锁会话里重新测。
教训：**行为断言的仪器要先校准**——先确认"已知有窗口时计数为 1"，再拿它去判断"关窗后是否为 0"。
