...
> **mac 版本序列自 4.2.0 起继续。** 此前 `v4.1.1`–`v4.1.8` 这 8 个版本号曾被 Android
> 里程碑（M1–M7）借用，CHANGELOG 中的这 8 条记录已更名为对应的
> `android-v0.1.0`–`android-v0.7.0`。两端版本号自此互不相干，不要再交叉编号——
> 交叉编号会产生「同一版本号在两端指不同东西」的歧义，事后清理成本很高。
