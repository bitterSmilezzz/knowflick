...
# 第二阶段（续作 · v3.9.1 → v4.1.0，2026-09-12）

第一阶段收官后，用户要求继续，遂启动第二阶段消化遗留事项，共 5 轮：

| 轮 | 版本 | 主题 | 关键产出 |
|---|------|------|---------|
| R21 | v3.9.1 | 批量生成与测试基建 | AI 生成超 6 张自动分批请求（跨批去重口径衔接）；重试退避可注入；**根治长期偶发的测试进程段错误**（崩溃报告定位：单测真实驱动系统 TextToSpeech 引擎，拆解期悬空回调） |
| R22 | v4.0.0 | 全包 Swift 6 | 应用层迁至严格并发，整个 Package 三 target 全部 Swift 6；实际迁移面远小于预估（仅 2 处修复） |
| R23 | v4.0.1 | 组件归位 | PressableButtonStyle 迁入设计系统文件；GlassIconButton 统一弹窗返回/关闭按钮 |
| R24 | v4.0.2 | 交叉复审 | 修复分批重构引入的两个 P2 提示词语义回归（topicHint 弱化、`

` 转义变形）；CHANGELOG 结构修正；并发告警清零（全新构建实测） |
| R25 | v4.1.0 | 收官 | 本节更新 + 三重验证 |

## 第二阶段特别记录

- **CHANGELOG 污染事故（R21 发现、同轮修复）**：v3.6.2 起的发布脚本用无锚定 `str.replace` 插入 `[Unreleased]` 段，每轮对全部匹配点复制，Unreleased 头指数增长至 256、文件膨胀至 3996 行且随 7 个提交累积。以 v3.6.1 为基线重建并重放全部条目（commit 0564ba5）。**教训：文档批量替换必须锚定（count=1）并在写入后做结构自检**；此后脚本均带断言。
- **测试基建稳定性**：困扰多轮的间歇性 signal 11（swift-testing runner 崩溃）通过 .ips 崩溃报告定位到系统 TextToSpeech 框架的异步回调在测试拆解期悬空；语音服务新增测试替身开关后连续多轮零崩溃，全套件时长稳定在 ~1.1s。
- **第二阶段复审结论**：无 P0/P1；P2 两项均为提示词搬运疏忽，已修复；「分批后所有批全废才报 noUsableCards」「BatchStub 样板重复」等经评估接受并记录。

## 两阶段累计

- 版本：v3.2.0 → v4.1.0（26 个 tag），测试 141 → 173 项 / 25 个 suite
- Swift 6 严格并发覆盖全部三个 target；Release 构建零警告
- 72 项第一阶段审计发现 + 第二阶段复审发现全部闭环（修复/补测试/明确取舍）
