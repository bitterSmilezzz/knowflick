...
final class StatsCalculatorTests {
    ...
    @Test func testEmptyCardsYieldsZeroStats() {
        ...
        #expect(stats.likeRate == 0)
        #expect(stats.streakDays == 0)
        #expect(stats.categories.isEmpty)
    }

    @Test func testUnseenCardsNotCounted() {
    ...
    @Test func testCountsSplitByIntention() {
        let cards = [
        ...
        let stats = StatsCalculator.compute(from: cards, calendar: calendar)
        #expect(stats.seenCount == 4)    // 4 张带意图的都已刷；skip 计入已刷
        #expect(stats.likedCount == 2)   // 只算右划
        #expect(stats.skipCount == 1)    // 跳过单独计数，不污染不喜欢
        #expect(abs(stats.likeRate - 0.5) <= 0.001)
    }

    // MARK: - 连续天数

    @Test func testStreakTodayAndYesterday() {
    ...
    @Test func testStreakAllowsTodayGap() {
    ...
    @Test func testStreakTodayGapBeyondYesterdayBreaks() {
        let now = Date(timeIntervalSince1970: 1_752_870_000)
    ...
    @Test func testStreakCountsSameDayOnce() {
        // 同一天刷多张只算一天
        let now = Date(timeIntervalSince1970: 1_752_870_000)
        let cards = [
            makeCard(seenAt: now),
    ...
    @Test func testDailyCountsLastSevenDays() {
        let now = Date(timeIntervalSince1970: 1_752_870_000)
        let cards = [
        ...
        #expect(daily[6].count == 2)    // 今天
        #expect(daily[5].count == 1)    // 昨天
        #expect(daily[2].count == 1)    // 4 天前
        #expect(daily[0].count == 0)    // 6 天前
        // 顺序从最旧到最新
        #expect(daily[0].day < daily[6].day)
    }

    // MARK: - 分类统计

    @Test func testCategoryGroupingSortedBySeenDesc() {
        let cards = [
        ...
        #expect(stats.categories.first?.category == "生物")
        #expect(stats.categories.first?.seen == 3)
        #expect(stats.categories.first?.liked == 1)   // skip 不进入喜欢
...
extension StatsCalculatorTests {
    @Test func nonpositiveDailyCountWindowIsEmpty() {
        #expect(StatsCalculator.dailyCounts(cards: [], days: -1).isEmpty)
        #expect(StatsCalculator.dailyCounts(cards: [], days: 0).isEmpty)
    }
}
